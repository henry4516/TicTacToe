...
while running:
                ...
                if not end:
                    if current_player == ttt.X or not AI: 
                        if board_left < x < board_right and board_up < y < board_down:
                            ...
                            ### successful move
                            if game.make_move(current_player, (grid_row, grid_col)):
                                round += 1
                                empty_grids -= 1
    ...
    if not end:
        if AI and current_player == ttt.O:
            AImove, _ = ttt.pruning(game, ttt.O)
            #print("AI move: ", AImove)
            game.make_move(ttt.O, AImove)
            current_player = ttt.X
            empty_grids -= 1
    ...
    if AI:
        ...
        render_text(f"AI thinking limit: {ttt.DEPTH_LIMIT}", black, AIonoff_left - 200, AIonoff_height + 50)
    else:
